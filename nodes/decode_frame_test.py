...
def test_decode_frame_by_name():
    ...
    result = decode_frame(
        ax, DecodeFrameInput(database=DB, message_name="EngineData", frame_id=-1, data_hex="a00f410000000000")
    )
...
def test_decode_frame_hex_prefix_and_separators_tolerated():
    ...
    result = decode_frame(
        ax,
        DecodeFrameInput(
            database=DB, message_name="EngineData", frame_id=-1, data_hex="0xA0:0F:41:00:00:00:00:00"
        ),
    )
    assert result.error.code == ""
    assert abs(result.signal_values["EngineSpeed"] - 1000.0) < 1e-6
...
def test_decode_frame_value_table_label():
    ax = AxiomTestContext()
    result = decode_frame(ax, DecodeFrameInput(database=DB, message_name="GearStatus", frame_id=-1, data_hex="02"))
    assert result.error.code == ""
...
def test_decode_frame_multiplexed_only_returns_active_group():
    ...
    result = decode_frame(
        ax, DecodeFrameInput(database=DB, message_name="MuxedFrame", frame_id=-1, data_hex="017b000000000000")
    )
    assert result.error.code == ""
...
def test_decode_frame_wrong_length_is_a_structured_error():
    ax = AxiomTestContext()
    result = decode_frame(ax, DecodeFrameInput(database=DB, message_name="EngineData", frame_id=-1, data_hex="aabb"))
...
def test_decode_frame_malformed_hex_is_a_structured_error():
    ax = AxiomTestContext()
    result = decode_frame(
        ax, DecodeFrameInput(database=DB, message_name="EngineData", frame_id=-1, data_hex="not-hex-zz")
    )
...
def test_decode_frame_message_not_found_is_a_structured_error():
    ax = AxiomTestContext()
    result = decode_frame(
        ax, DecodeFrameInput(database=DB, message_name="NoSuchMessage", frame_id=-1, data_hex="00")
    )
...
def test_encode_then_decode_round_trip_matches_original_values():
    ...
    decoded = decode_frame(
        ax, DecodeFrameInput(database=DB, message_name="EngineData", frame_id=-1, data_hex=encoded.data_hex)
    )

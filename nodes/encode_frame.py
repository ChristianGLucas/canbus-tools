...
def encode_frame(ax: AxiomContext, input: EncodeFrameInput) -> EncodeFrameOutput:
    ...
    frame_id (leave the other unset). In strict mode, a signal value
    outside its declared [min, max] range fails with OUT_OF_RANGE instead
    of being encoded silently; an unknown signal name always fails with
    SIGNAL_NOT_FOUND rather than being silently ignored.
    """
    try:
